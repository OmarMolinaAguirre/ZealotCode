...
@Autonomous(name="Test New Turn Counter", group="Zealot")
@Disabled
public class Test_TurnCounter extends LinearOpMode {
...
// Test Methods
    public void DriveForwardByTurns(int turns) {
        ...
        while (opModeIsActive() && (turnCount < turns)) {
            ...
            telemetry.addData("Touch Pressure", robot.turnSensor.getValue());
            telemetry.update();
            if (robot.turnSensor.getValue() > .01) {
                turnCount++;
            }
        }
        StopDriving();
